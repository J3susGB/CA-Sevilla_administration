import { Injectable } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { Observable } from 'rxjs';

export interface TestNote {
  id?: number;
  arbitro_id: number;
  nif: string;
  nota: number;
  categoria_id: number;
  name: string;
  first_surname: string;
  second_surname?: string;
}

export interface TestSession {
  id: number;
  fecha: string;        // "30-05-2025"
  testNumber: number;
  categoria: string;    // "Provincial", etc.
  categoria_id: number;
  notas: TestNote[];
}

@Injectable({ providedIn: 'root' })
export class TestService {
  private readonly API_URL = 'http://localhost:8000/api/tests';

  constructor(private http: HttpClient) {}

  listAll(): Observable<{status:string; data:TestSession[]}> {
    return this.http.get<any>(this.API_URL);
  }
  get(id: number): Observable<{status:string; data:TestSession}> {
    return this.http.get<any>(`${this.API_URL}/${id}`);
  }
  totals(): Observable<{status:string; data:any[]}> {
    return this.http.get<any>(`${this.API_URL}/totals`);
  }
  create(body: {
    nif: string;
    nota: number;
    categoria_id: number;
    /** o bien */
    sessionId?: number;
    /** o bien */
    fecha?: string;
    testNumber?: number;
  }): Observable<any> {
    return this.http.post(this.API_URL, body);
  }
  update(id: number, body: any): Observable<any> {
    return this.http.put<any>(`${this.API_URL}/${id}`, body);
  }
  delete(id: number): Observable<any> {
    return this.http.delete<any>(`${this.API_URL}/${id}`);
  }
  bulkUpload(form: FormData): Observable<any> {
    return this.http.post<any>(`${this.API_URL}/bulk-upload`, form);
  }
  truncate(): Observable<any> {
    return this.http.post<any>(`${this.API_URL}/truncate`, {});
  }
}
